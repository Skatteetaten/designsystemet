...
export const FileUploaderTest = () => {
  ...
  const handleChange = async (files: File[]): Promise<void> => {
    try {
      ...
      // Setter tilstanden til vellykket opplasting med filene
      const uploadedFiles = files.map((file, index) => ({
        name: file.name,
        href: results[index].fileUrl,
        id: results[index].fileId,
      }));

      setSuccess(uploadedFiles);
    } catch (error: unknown) {
      // Håndterer feil under opplastingen
      setFailure(
...
Når du bruker FileUploader med `multiple={true}`, kan du ha behov for å begrense antall filer eller filtyper:

```jsx
const handleChange = async (files: File[]): Promise<void> => {
  // Sjekk for maksimalt antall filer
  if (fileUploaderState.uploadedFiles.length + files.length > 5) {
    setErrorMessage('Du kan maks laste opp 5 filer');
  }

  // Fortsett med opplastingen
  setLoading();
  // ...
};
```

### Komplett feilhåndteringseksempel
...
### Delvis vellykket opplasting

Når noen filer lykkes og andre feiler, kan du bruke det valgfrie `succeededFiles`-parameteret:

```jsx
const results = await Promise.allSettled(uploadPromises);

const succeeded: Array<UploadedFile> = [];
const failed: Array<UploadedFile> = [];

// Separer vellykkede og mislykkede opplastinger
results.forEach((result, index) => {
  if (result.status === 'fulfilled') {
    succeeded.push({ name: files[index].name, id: result.value.id });
  } else {
    failed.push({ name: files[index].name });
  }
});

// Rapporter både feil og vellykkede opplastinger
if (failed.length > 0) {
  setFailure(
    failed,
    [{ error: 'Opplasting feilet', files: failed }],
    succeeded // Legg til vellykkede filer i tilstanden
  );
} else {
  setSuccess(succeeded);
}
