import {
  Meta,
  Primary,
  Controls,
  ArgTypes,
} from '@storybook/addon-docs/blocks';
import { PlayroomCanvas as Canvas } from '../../playroom/utils';
import * as SelectStories from './Select.stories';
import * as SelectOption from './SelectOption.stories';
import { VersionBadge } from '../utils/VersionBadge';
import { DocIntro } from '../utils/DocIntro';
import { DocHeaderWrapper } from '../utils/DocHeaderWrapper';

<Meta of={SelectStories} />

<DocHeaderWrapper componentName="Select">

# Select

<VersionBadge packageName={'forms'} />

<DocIntro url="https://www.skatteetaten.no/stilogtone/designsystemet/komponenter/select/" />

</DocHeaderWrapper>

<Primary />

```tsx
import { Select } from '@skatteetaten/ds-forms';
```

<Controls />

## `Select.Option`

Select.Option støtter generisk typing av value. Dette er nyttig for å avgrense verdier.

```tsx
type Fruit = '' | 'banan' | 'eple' | 'kiwi' | 'pære' | 'sitron';

<Select>
  <Select.Option<Fruit> value={'banan'}>{'Banan'}</Select.Option>
  <Select.Option<Fruit> value={'eple'}>{'Eple'}</Select.Option>
  <Select.Option<Fruit> value={'kiwi'}>{'Kiwi'}</Select.Option>
  <Select.Option<Fruit> value={'pære'}>{'Pære'}</Select.Option>
  <Select.Option<Fruit> value={'sitron'}>{'Sitron'}</Select.Option>
</Select>;
```

<ArgTypes of={SelectOption} />

## Examples

<Canvas of={SelectStories.Examples} />
