...
- **OpenClose, TextField, Select, DatePicker, SearchField og Combobox**: `variant` er fjernet. Standard skjemafelt har bare én størrelse, og unntakene SearchField og Combobox bruker nå `size` i stedet.
- **Skjemakomponenter**: `showRequiredMark` er fjernet fra alle komponenter. (Varslet i versjon 2.3.0.)
- **Table**: `variant` er fjernet. Bruk `size` i stedet.
...
- **Fargeprops (color)**: Fargeverdier er harmonisert på tvers av flere komponenter, blant annet Link, LinkGroup og Accordion.
...
- **StepList**: Justert autofokus slik at første steg ikke lenger får uønsket fokus.
- **Table** (redigerbar rad): Forbedret skjermleserinstruks for redigerknapp plassert til venstre.
- **FileUploader**: Feilmeldinger ved opplasting av flere filer leses nå korrekt opp når multiple er satt til false.
...
- **DescriptionList**: Rettet inkonsistent bruk av margin og padding ved `hasSpacing`.
...
- **Hjelpetekst**: Oppdatert kantlinje slik at den samsvarer bedre med Popover og Card.
