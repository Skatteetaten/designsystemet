...
export const useFileUploader = <T,>(
  ...
  const setSuccess: SuccessMethod<T> = useCallback(
    (files, data) => {
  ...
  const setLoading: LoadingMethod = useCallback(() => {
    setUploadResult(undefined);
    setIsUploading(true);
  }, []);

  const setFailure: FailureMethod<T> = useCallback(
    (files, errorMessage, succeeded?) => {
      setIsUploading(false);
  ...
  const remove: RemoveMethod = useCallback((file) => {
    dispatch({ type: FileActionType.REMOVE, file });
    setUploadResult(undefined);
  }, []);

  const renderFileStatusMessage = useCallback(
    (status: UploadResultData<T>): ReactNode => {
      if (renderStatus) {
  ...
  const uploaderState = useMemo(
    ...
    [state, uploadResult, isUploading, renderFileStatusMessage]
  );

  return useMemo(
    () => [uploaderState, setSuccess, setLoading, setFailure, remove] as const,
    [uploaderState, setSuccess, setLoading, setFailure, remove]
  );
};
