...
export type UploadResult = {
  /**
   * Etter at en eller flere filer ble forsøkt lastet opp skal bruker få en
   * oppsummering av hvordan opplastingen gikk.
   */
  statusMessage?: ReactNode;
  /**
   * Styrer visning av Alert som indikerer om opplastingen var vellykket eller
   * ikke
   */
...
export interface FileUploaderCommonProps
  ...
  >;
  /**
   * Styrer hvilke filformater som er mulig å velge i operativsystemets
   * filutforsker. Med Drag-and-drop vil det fortsatt være mulig å slippe inn
   * alle filer.
   */
  ...
  /**
   * Overskriver default tittel på fil-ikonet som brukes i listen med filer som
   * er lastet opp
   */
  fileIconTitle?: string;
  /**
   * Liste med opplastede filer som skal vises under filopplasteren. Det er
   * mulig å angi href dersom filen skal kunne lastes ned igjen. Det er også
   * mulig å angi en id som brukes til key i rendring av lista med filer. Hvis
   * id ikke blir satt så faller komponenten tilbake på filnavn. Det betyr at
   * dersom det er behov for å kunne laste opp flere ulike filer med identisk
   * filnavn så er det nødvendig å anngi unik id.
   */
  uploadedFiles?: Array<UploadedFile>;
  /**
   * Etter at en eller flere filer ble forsøkt lastet opp skal bruker få en
   * oppsummering av hvordan opplastingen gikk. uploadResult.hasUploadFailed
   * angir om opplasting var vellykket og uploadResult.statusMessage brukes til
   * å vise en melding om hvordan det gikk med opplastingen.
   */
  ...
  isUploading?: boolean;
  /**
   * Erstatter tegn som er ugyldige. Bruk invalidCharacterRegexp for å overstyre
  ...
  /**
   * Kalles når fil skal lastes ned. Brukes om man trenger å overstyre standard
   * oppførsel til lenkene og initiere nedlasting selv.
   *
...
export interface UploadedFile {
  ...
  href?: string;
  /**
   * Brukes som key i rendring av lista med filer og til å oppdage duplikat av
   * fil. Komponenten faller tilbake på filnavn som key hvis man ikke oppgir
   * id.
   */
  id?: string;
}
