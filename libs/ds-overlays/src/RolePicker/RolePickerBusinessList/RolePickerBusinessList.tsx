...
export const RolePickerBusinessList = ({
  ...
  const hasItemsWithSubOranizations = businesses.list.some(
    (business: Business) => business.subunits
  );
  ...
  const getShowAllCount = useCallback((): number => {
    ...
    if (showSubUnits) {
      return businesses.list
        .filter((item: Business) => !item.isDeleted)
        .reduce((total: number, curr: Business) => {
          const subUnitsCount =
            curr.subunits?.filter((sub: Business) => !sub.isDeleted).length ??
            0;
    ...
    return businesses.list.filter((item: Business) => !item.isDeleted).length;
